...
module WelcomeHelper
  ...
  # List of actions with Icons. They have to be declared in image_
  Icons = {
    :destroy => :delete,
    :edit => :edit,
    ...
    elt.last.each do |action|
      ...
      Icons.each { |i| case action.intern; when *(i.first)
          if i.last.is_a? Array
            icons.push link_to(send("image_#{i.last.first}"), options)
          else
            icons.push send("image_#{i.last}")
          end
        end
      }
    end
  ...
  def preview_image(theme)
    image = image_tag("screenshots/#{theme}_preview.png",
                      image_options(_('%s theme') % theme, '258x142'))
